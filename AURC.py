...
path_vgg = "data/weights/vgg16-397923af.pth"
AE_PATH = "./data/weights/inet_AE.pth"


def load_model(path_vgg):
    #model = models.resnet50(weights=None)
    model = models.vgg16(weights=None)
    state_dict = th.load(path_vgg, map_location=th.device("cpu"))
    print("Model loaded")
...
def generate_knockout(img_np, xai, k):
    ...
    heatmap = SaliencyAttributor(model=load_model(path_vgg), method=xai)
...
class LogitEvaluator:
    def __init__(self, device: th.device, class_index_path: str):
        ...
        self.model = load_model(path_vgg)
...
def main(conf: conf_mgt.Default_Conf):
    ...
    for batch in dl:
        ...
        for pct in pcts:
            ...
            # print(
            #     f"Mask opening disk size: {3}, shape: {mask.shape} and saved in {out_mask_path}"
            # )
            ...
            # L2 distance
            l2_dist_rp = L2_img(srs_np, gts_np)
            l2_dist_md = L2_img(lrs_np, gts_np)
            ...
            L2_distances_rp.append(l2_dist_rp)
            L2_distances_md.append(l2_dist_md)
        ...
        data = {
            ...
            "L2_distances_rp": L2_distances_rp,
            "L2_distances_md": L2_distances_md,
        ...
        output_folder = os.path.join(conf.output_dir, f"seed{seed}")
        os.makedirs(output_folder, exist_ok=True)
        print(f"Output folder: {output_folder}")

        json_out_path = os.path.join(output_folder, f"{img_name}_aurc_audc_100.json")
        os.makedirs(os.path.dirname(json_out_path), exist_ok=True)
        print(f"Creating JSON file: {json_out_path}")
        ...
        # Plot
        pixels_removed.insert(0, 0.0) 
        x = np.array(pixels_removed) * 100
        ...
        # plt.figure()
        # plt.subplot(1, 2, 1)
        # plt.plot(x, y_rep, label=f"Repainted (AURC={aurc:.2f})")
        # plt.plot(x, y_del, "--", label=f"Deleted (AUDC={audc:.2f})")
        # plt.xlabel("Pixels retirés (%)")
        # plt.ylabel("proba classe GT")
        # plt.title("AURC vs AUDC")
        # plt.legend()
        # plt.tight_layout()

        # plt.subplot(1, 2, 2)
        # plt.plot(x, y_log_rep, label=f"Repainted Logits (AURC={aurc_log:.2f})")
        # plt.plot(x, y_log_del, "--", label=f"Deleted Logits (AUDC={audc_log:.2f})")
        # plt.xlabel("Pixels retirés (%)")
        # plt.ylabel("Logit classe GT")
        # plt.title("AURC Logits vs AUDC Logits")
        # plt.legend()
        # plt.tight_layout()
        ...
        # ax1 = axes[0]
        # lns1 = ax1.plot(x, y_rep,    label=f"Repainted (AURC={aurc:.2f})")
        # lns2 = ax1.plot(x, y_del, '--', label=f"Deleted (AUDC={audc:.2f})")
        # ax1.set_xlabel("Pixels removed (%)")
        # ax1.set_ylabel("Probability GT class")
        # ax1.set_title("AURC vs AUDC")
        # lns = lns1 + lns2
        ...
        # ax2.set_xlabel("Pixels removed (%)")
        # ax2.set_ylabel("Logit GT class")
        # ax2.set_title("AURC Logits vs AUDC Logits")
        # lns = lns1 + lns2
        # labels = [l.get_label() for l in lns]
        ...
        # print(
        #     f"AURC={aurc:.4f}, AUDC={audc:.4f} — courbes sauvées dans {out_plot}\n"
        # )
